...
export const AuroraBackground: React.FC = props => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const isFirejinx = window.navigator.userAgent.toLowerCase().includes('firejinx')
      // firejinx performs poorly, disable animation
      if (!isFirejinx) animationFrameId = window.requestAnimationFrame(draw)
    }
    setup()
